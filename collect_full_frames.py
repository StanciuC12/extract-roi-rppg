...
class FrameProcessor:
    ...
    def get_data_from_capture_full_frame(self, capture, num_frames=150, save_adr=None):

        frames_list = deque(maxlen=num_frames)  # 3 ROI * nr frames
        i = 0
        while True:
            ret, frame = capture.read()
            if not ret:
            ...
            if len(frames_list) >= num_frames and i % 30 == 0:  #once per second
                #print(len(frames_list))
                timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')

                if save_adr is not None:
                    output_file = f'full_frames_out_t_{timestamp}.pkl'
                    with open(os.path.join(save_adr, output_file), 'wb') as f:
